...
import (
	...
	"github.com/RobyFerro/go-web-framework/http/controller"
	"github.com/RobyFerro/go-web-framework/http/middleware"
...
var (
	...
	Configuration, _ = config.Configuration()
...
func WebRouter() (*mux.Router, error) {
	...
	routes, err := config.ConfigurationWeb()
	if err != nil {
		return nil, err
	}
...
func GetControllerInterface(directive []string, w http.ResponseWriter, r *http.Request) interface{} {
	...
	for _, contr := range controller.Controllers {
...
func registerBaseController(res http.ResponseWriter, req *http.Request, controller *interface{}) *interface{} {
	...
	if err := checkControllerIntegrations(&BaseController); err != nil {
		exception.ProcessError(err)
	}
...
func checkControllerIntegrations(base *controller.BaseController) error {
